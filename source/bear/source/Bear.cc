...
namespace bear {
    ...
    Command::Command(const sys::Process::Builder &intercept, const sys::Process::Builder &citnames, fs::path output) noexcept
            : ps::Command(), intercept_(intercept), citnames_(citnames), output_(std::move(output)) {}

    [[nodiscard]] rust::Result<int>
    ...
    rust::Result<flags::Arguments>
    Application::parse(int argc, const char **argv) const {
    ...
    rust::Result<ps::CommandPtr>
    Application::command(const flags::Arguments &args, const char **envp) const {
